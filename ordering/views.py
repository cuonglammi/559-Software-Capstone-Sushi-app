        ...
        def wrapped(request, *args, **kwargs):
            if request.user.role != role:
                return render(
                    request,
                    "ordering/error.html",
                    {"message": "Your staff role cannot access this page."},
                    status=403,
                )
...
def sign_in(request):
    ...
    if request.method == "POST":
        ...
        key = hashlib.sha256(
            request.POST.get("username", "").strip().encode()
        ).hexdigest()
        attempt, _ = LoginAttempt.objects.get_or_create(
            key=key, defaults={"window_started_at": timezone.now()}
        )
        ...
        if attempt.failures >= 5:
            form.add_error(
                None, "Too many sign-in attempts. Please try again in 15 minutes."
            )
        elif form.is_valid():
...
def password(request):
    ...
    return render(
        request,
        "ordering/form.html",
        {"form": form, "title": "Change password", "button": "Update password"},
    )
...
def tables(request):
    sessions = (
        TableSession.objects.filter(ended_at__isnull=True)
        .select_related("table", "server")
        .prefetch_related("orders__items")
    )
    return render(
        request,
        "ordering/tables.html",
        {
            "sessions": sessions,
...
def new_session(request):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        try:
            ...
            form.add_error(
                "table",
                "This table was just opened by another server. Choose another table.",
            )
        else:
            messages.success(
                request,
                f"Table {dining.table.number} is ready. Open the customer menu link on the table device.",
            )
            return redirect("tables")
    return render(
        request,
        "ordering/form.html",
        {"form": form, "title": "Start a table session", "button": "Open table"},
    )
...
def end_session(request, pk):
    ...
    try:
        close_session(pk)
        messages.success(
            request, "Table session closed. The customer link no longer accepts orders."
        )
...
def kitchen(request):
    orders = (
        ...
        .filter(session__ended_at__isnull=True)
...
def queue_data(request):
    orders = Order.objects.filter(session__ended_at__isnull=True).select_related(
        "session__table"
    )
    return JsonResponse(
        {
            "orders": [
                {"id": o.pk, "table": o.session.table.number, "status": o.status}
                for o in orders
            ]
        }
    )
...
def dining_session(token):
    return get_object_or_404(
        TableSession.objects.select_related("table"), access_token=token
    )
...
def add_item(request, token, pk):
    ...
    if dining.ended_at:
        messages.error(
            request, "This table session has ended. Please ask your server for help."
        )
    elif not item.available:
        messages.error(request, "This item is currently unavailable.")
    elif form.is_valid():
        cart = request.session.get(cart_key(dining), {})
        quantity = form.cleaned_data["quantity"] + cart.get(str(pk), {}).get(
            "quantity", 0
        )
        if quantity > 10:
            messages.error(
                request,
                "You can order up to 10 of each item per order. Edit the quantity in your cart.",
            )
        elif len(cart) >= 100 and str(pk) not in cart:
            messages.error(
                request,
                "Your cart is full. Submit this order before adding more items.",
            )
        else:
        ...
        messages.error(
            request, "Use a quantity from 1 to 10 and notes up to 500 characters."
        )
...
def cart(request, token):
    ...
    context = customer_context(request, dining)
    context.update(
        {"rows": rows, "total": total, "submission_key": request.session[key]}
    )
...
def checkout(request, token):
    ...
    try:
        order = submit_order(
            dining.pk, request.session.get(cart_key(dining), {}), uuid.UUID(submitted)
        )
    except ValidationError as exc:
        messages.error(request, exc.messages[0])
        return redirect("cart", token=token)
        ...
        messages.error(
            request, "Unable to submit order. Your cart is saved; please try again."
        )
        return redirect("cart", token=token)
...
def confirmation(request, token, pk):
    ...
    order = get_object_or_404(
        Order.objects.prefetch_related("items"), pk=pk, session=dining
    )
    context = customer_context(request, dining)
    context["order"] = order
...
def error_404(request, exception):
    return render(
        ...
        "ordering/error.html",
        {
            "message": "We could not find that page. Please check your table link or ask a server."
        },
        status=404,
    )
