...
class WorkflowTests(TestCase):
    ...
    def setUpTestData(cls):
        cls.server = Staff.objects.create_user(username='server01', password='TestPass1!', full_name='Alex', role='server')
        cls.kitchen = Staff.objects.create_user(username='kitchen01', password='TestPass1!', full_name='Jamie', role='kitchen')
        cls.table = RestaurantTable.objects.create(number=12, capacity=4)
        cls.dining = TableSession.objects.create(table=cls.table, server=cls.server, customers=2)
        category = MenuCategory.objects.create(name='Rolls')
        cls.roll = MenuItem.objects.create(category=category, name='California Roll', description='Crab and avocado', price='8.99')
        cls.salmon = MenuItem.objects.create(category=category, name='Salmon Roll', description='Fresh salmon', price='5.99')
    ...
    def test_login_throttled_after_five_failures(self):
        for _ in range(5):
            self.client.post('/login/', {'username': 'server01', 'password': 'wrong'})
        response = self.client.post('/login/', {'username': 'server01', 'password': 'TestPass1!'})
    ...
    def test_idle_timeout_and_background_poll(self):
        ...
        session.save()
        self.assertEqual(self.client.get('/api/queue/', HTTP_X_BACKGROUND_POLL='1').status_code, 302)
        self.assertNotIn('_auth_user_id', self.client.session)

    def test_profile_cannot_escalate_role(self):
        self.client.force_login(self.server)
        self.client.post('/profile/', {'full_name': 'Alex Nguyen', 'role': 'kitchen', 'is_superuser': 'true'})
    ...
    def test_password_change_and_hashing(self):
        self.client.force_login(self.server)
        data = {'old_password': 'wrong', 'new_password1': 'NewPass1!', 'new_password2': 'NewPass1!'}
    ...
    def test_capacity_and_duplicate_active_session(self):
        ...
        with self.assertRaises(IntegrityError), transaction.atomic():
            TableSession.objects.create(table=self.table, server=self.server, customers=1)
        self.client.force_login(self.server)
        self.client.post('/tables/new/', {'table': table.pk, 'customers': 2, 'ayce': 'on', 'server': self.kitchen.pk})
    ...
    def test_search_filter_sort_and_sql_injection(self):
        ...
        self.client.post('/login/', {'username': "' OR 1=1 --", 'password': 'TestPass1!'})
        self.assertNotIn('_auth_user_id', self.client.session)
    ...
    def test_cart_total_ayce_and_quantity_bounds(self):
        ...
        for quantity in [0, 11, -1, 1.5, True]:
            with self.subTest(quantity=quantity), self.assertRaises(ValidationError):
                submit_order(self.dining.pk, {str(self.roll.pk): {'quantity': quantity, 'notes': ''}}, uuid.uuid4())

    def test_price_snapshot_idempotency_and_transitions(self):
        key = uuid.uuid4()
        order = submit_order(self.dining.pk, self.cart_data(), key)
        self.assertEqual(submit_order(self.dining.pk, self.cart_data(), key).pk, order.pk)
    ...
    def test_complete_customer_post_workflow_and_retry(self):
        ...
        self.client.post(reverse('add_item', args=[token, self.roll.pk]), {'quantity': 2, 'notes': '<script>alert(1)</script>'})
        ...
        response = self.client.post(reverse('checkout', args=[token]), {'submission_key': key}, follow=True)
    ...
    def test_failed_checkout_preserves_cart(self):
        token = self.dining.access_token
        self.client.post(reverse('add_item', args=[token, self.roll.pk]), {'quantity': 1, 'notes': ''})
        self.client.get(reverse('cart', args=[token]))
        key = self.client.session[f'submission_{self.dining.pk}']
        with patch('ordering.views.submit_order', side_effect=RuntimeError('private database detail')), self.assertLogs('ordering.views', level='ERROR'):
            response = self.client.post(reverse('checkout', args=[token]), {'submission_key': key}, follow=True)
    ...
    def test_customer_links_are_scoped(self):
        other = TableSession.objects.create(table=RestaurantTable.objects.create(number=99, capacity=2), server=self.server, customers=1)
        order = submit_order(self.dining.pk, self.cart_data(), uuid.uuid4())
        self.assertEqual(self.client.get(reverse('confirmation', args=[other.access_token, order.pk])).status_code, 404)
        self.assertEqual(self.client.get(reverse('menu', args=[uuid.uuid4()])).status_code, 404)
        self.assertEqual(self.client.post('/kitchen/1/status/', {'status': 'completed'}).status_code, 302)

    def test_csrf_and_security_headers(self):
        client = Client(enforce_csrf_checks=True)
        self.assertEqual(client.post('/login/', {'username': 'server01', 'password': 'TestPass1!'}).status_code, 403)
    ...
    def test_https_and_cookie_flags(self):
        ...
        response = self.client.post('/login/', {'username': 'server01', 'password': 'TestPass1!'}, secure=True)
    ...
    def test_cart_edit_remove_and_invalid_input(self):
        ...
        self.client.post(edit, {'quantity': 3, 'notes': 'No sesame', 'action': 'update'})
