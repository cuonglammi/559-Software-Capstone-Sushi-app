...
def cart_rows(cart, table_session):
    ...
    for key, value in cart.items():
        if key in items:
            ...
            rows.append({'item': item, 'quantity': value['quantity'], 'notes': value['notes'],
                         'price': price, 'subtotal': price * value['quantity']})
...
def submit_order(session_id, cart, submission_key):
    ...
    existing = Order.objects.filter(submission_key=submission_key, session=dining).first()
    if existing:
        return existing
    if dining.ended_at:
        raise ValidationError('This table session has ended. Please ask your server for help.')
    if not cart:
    ...
    for value in cart.values():
        if (type(value.get('quantity')) is not int or not 1 <= value['quantity'] <= 10
                or not isinstance(value.get('notes'), str) or len(value['notes']) > 500):
            raise ValidationError('Check item quantities (1–10) and notes (up to 500 characters).')
    rows, _ = cart_rows(cart, dining)
    if len(rows) != len(cart) or any(not r['item'].available for r in rows):
        raise ValidationError('An item is no longer available. Remove it from your cart and try again.')
    for row in rows:
        if type(row['quantity']) is not int or not 1 <= row['quantity'] <= 10 or len(row['notes']) > 500:
            raise ValidationError('Check item quantities (1–10) and notes (up to 500 characters).')
    order = Order.objects.create(session=dining, submission_key=submission_key)
    OrderItem.objects.bulk_create([OrderItem(order=order, menu_item=r['item'], name=r['item'].name,
        quantity=r['quantity'], notes=r['notes'], unit_price=r['price']) for r in rows])
    return order

@transaction.atomic
def transition_order(order_id, status):
    order = Order.objects.select_for_update().get(pk=order_id)
    next_status = {Order.Status.PENDING: Order.Status.IN_PROGRESS, Order.Status.IN_PROGRESS: Order.Status.COMPLETED}
    if next_status.get(order.status) != status:
        raise ValidationError('This order has changed. Refresh the queue and use the next available action.')
    order.status = status
...
def close_session(session_id):
    ...
    if dining.ended_at:
        return
    if dining.orders.filter(status__in=[Order.Status.PENDING, Order.Status.IN_PROGRESS]).exists():
