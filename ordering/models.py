    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                condition=Q(capacity__gte=1), name="table_capacity_positive"
            )
        ]
...
class TableSession(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=Q(customers__gte=1), name="session_customers_positive"
            ),
        ]
...
class MenuItem(models.Model):
    ...
    illustration = models.CharField(
        max_length=12,
        default="roll",
        choices=[(x, x.title()) for x in ["roll", "nigiri", "edamame", "soup", "tea"]],
    )
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(
                condition=Q(price__gte=0), name="menu_price_nonnegative"
            )
        ]
...
class Order(models.Model):
    ...
    session = models.ForeignKey(
        TableSession, on_delete=models.PROTECT, related_name="orders"
    )
...
class OrderItem(models.Model):
    ...
    preparation_status = models.CharField(
        max_length=15, choices=Order.Status.choices, default=Order.Status.PENDING
    )

    class Meta:
        constraints = [
            models.CheckConstraint(
                condition=Q(quantity__gte=1, quantity__lte=10),
                name="order_quantity_range",
            ),
            models.CheckConstraint(
                condition=Q(unit_price__gte=0), name="order_price_nonnegative"
            ),
        ]
