...
@skipUnless(os.getenv('RUN_BROWSER_TESTS') == '1', 'Set RUN_BROWSER_TESTS=1 to run browser tests')
class BrowserWorkflowTests(StaticLiveServerTestCase):
    ...
    def assert_layout(self, page):
        self.assertTrue(page.evaluate('document.documentElement.scrollWidth <= innerWidth'), 'Horizontal overflow')
        self.assertEqual(page.locator('img').evaluate_all('(imgs) => imgs.filter(i => !i.complete || i.naturalWidth === 0).length'), 0)
    ...
    def test_server_customer_kitchen_and_closed_session(self):
        ...
        server.get_by_label('Table:', exact=True).select_option(label='Table 12 · 4 seats')
        server.get_by_label('Number of guests').fill('2')
        server.get_by_role('button', name='Open table', exact=True).click()
        expect(server.get_by_role('heading', name='Table 12', exact=True)).to_be_visible()
        ...
        expect(customer.get_by_role('heading', name='California Roll', exact=True)).to_be_visible()
        ...
        expect(customer.get_by_role('heading', name='Your order is in!')).to_be_visible()
        order = self.db(lambda: Order.objects.get(session=dining))
        expect(kitchen.get_by_role('button', name=f'Start preparing order #{order.pk}', exact=True)).to_be_visible(timeout=12000)
        kitchen.get_by_role('button', name=f'Start preparing order #{order.pk}', exact=True).click()
        kitchen.get_by_role('button', name=f'Complete order #{order.pk}', exact=True).click()
        expect(kitchen.get_by_text('Ready to serve')).to_be_visible()
        server.reload()
        server.get_by_role('button', name='Close table session').click()
        customer.goto(menu_url)
        expect(customer.get_by_text('This table session has ended.', exact=False)).to_be_visible()
        self.assertEqual(customer.get_by_role('button', name='Add California Roll to cart').count(), 0)
    ...
    def test_responsive_login_menu_cart_and_profile(self):
        ...
        dining = self.db(lambda: TableSession.objects.create(table=RestaurantTable.objects.get(number=12), server=Staff.objects.get(username='server01'), customers=2, ayce=True))
        public = self.browser.new_page()
        for width, height in [(320, 700), (768, 1024), (1024, 768)]:
            for path in ['/login/', f'/table/{dining.access_token}/', f'/table/{dining.access_token}/cart/']:
        ...
        if target:
            ...
            public.screenshot(path=str(Path(target) / 'customer-mobile.png'), full_page=True)
            public.set_viewport_size({'width': 1280, 'height': 900})
            public.goto(self.live_server_url + '/login/')
            public.screenshot(path=str(Path(target) / 'staff-login.png'), full_page=True)
